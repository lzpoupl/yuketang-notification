...
window.addEventListener(
  "message",
  async function (e: MessageEvent<AjaxMessage>) {
    // console.log("收到消息: ", e.data);
    if (!e.data.url) return;
    const settings = (
      (await chrome.storage.local.get("settings")) as { settings: Settings }
    ).settings;
    if (!settings) return;
    if (
      e.data.url.includes("/api/v3/lesson/presentation/fetch") &&
      settings.autoAnswer
    ) {
      ...
      if (message.op) {
        ...
        if (message.op == "newdanmu") danmu = message.danmu;
      }
      if (problemId && settings.autoAnswer) {
        console.log("解锁问题: ", problemId);
        if (settings.notificationSound) {
          try {
            chrome.runtime.sendMessage({ type: "receiveQuestion" });
            await delay(500);
          } catch (e) {
            console.error(e);
          }
        ...
        const problems = (
          (await chrome.storage.local.get("problems")) as {
            problems: Problem[];
          }
        ).problems;
        ...
        if (problem && settings.autoAnswerTypes.includes(problem.problemType)) {
          ...
          if (problem.problemType === 1 || problem.problemType === 2) {
            try {
              ...
              try {
                const slides =
                  this.document.getElementsByClassName("timeline__item");
